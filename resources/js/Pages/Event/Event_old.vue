                    ...
                    <el-row>
                        <el-card class="w-100 mb-5 my-card" v-for="(t, index) in data.ticketsReserved" :key="index">
                            ...
                            <el-row :gutter="gutterValue">
                                ...
                                <el-col :xs="24" :sm="24" :md="12" :lg="8" :xl="8" class="mb-3" v-for="(question, iq) in t.questions" :key="question.id">
                                    <label class="bold has-text-dark">{{ question.title }} <span class="has-text-danger" v-if="question.required">*</span></label>
                                    <el-input
                                        class="el-form-item mb-0 mt-1"
                                        :class="{'is-error': errors.questions[index][iq]}"
                                        v-model="t.question[iq]"
            ...
            this.data.tickets.forEach((t, i) => {
                ...
                if (t.quantity > 0 && save) {
                    for (let j = 0; j < t.quantity; j++) {
                        ...
                        this.errors.questions.push([false, false, false, false, false]);
            ...
            this.data.ticketsReserved.forEach((t, i) => {
                ...
                if (t.questions.length) {
                    t.questions.forEach((q, j) => {
                        if (q.required && !t.question[j].length) {
                            this.errors.questions.push([]);
                            // console.log(t.question);
                            this.errors.questions[i][j] = true;
                            valid = false;
                            // console.log(`Campo ${q.title} es requerido.`);
                        }
                    });
                }
            });
            ...
            this.event.tickets.forEach(t => {
                if (t.questions.length) {
                    t.questions.forEach(q => {
                        q.options = q.type === 'select' && !Array.isArray(q.options) ? q.options.split(',') : q.options;
                    });
                }
